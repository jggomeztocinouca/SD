...
def add_miembro():
    ...
    if miembro['categoria'] == 'PDI':  # Si la categoría es PDI
        miembro['asignaturas'] = input('Asignaturas (separadas por comas): ').split(',')  # Pedimos las asignaturas y
        # las guardamos como una lista
    requests.post(base_url, json=miembro)  # Hacemos una petición POST a la URL base para añadir el nuevo miembro,
    # enviando los datos en el cuerpo de la petición
...
def get_miembro():
    ...
    respuesta = requests.get(
        f'{base_url}/{dni}')  # Hacemos una petición GET a la URL correspondiente para obtener el miembro
    print(json.dumps(respuesta.json(), indent=2))  # Imprimimos la respuesta, formateándola para que sea más legible
...
def modify_miembro():
    ...
    if miembro['categoria'] == 'PDI':  # Si la nueva categoría es PDI
        miembro['asignaturas'] = input('Asignaturas (separadas por comas): ').split(
            ',')  # Pedimos las nuevas asignaturas y las guardamos como una lista
    requests.put(f'{base_url}/{dni}',
                 json=miembro)  # Hacemos una petición PUT a la URL correspondiente para modificar el miembro,
    # enviando los nuevos datos en el cuerpo de la petición
...
def delete_miembro():
    ...
    requests.delete(
        f'{base_url}/{dni}')  # Hacemos una petición DELETE a la URL correspondiente para eliminar el miembro
