...
def reserve():
    ...
    for reserva in reservas.values():
        ...
        if reserva['pista'] == datos['pista'] and (
                reserva['inicio_hora'] <= datos['inicio_hora'] < reserva['fin_hora'] or reserva['inicio_hora'] < datos[
            'fin_hora'] <= reserva['fin_hora']):
    ...
    reservas[id] = {'id': id, 'pista': datos['pista'], 'inicio_hora': datos['inicio_hora'],
                    'fin_hora': datos['fin_hora'],
                    'player': datos['player']}  # Añadimos la nueva reserva al diccionario
    guardar_reservas()
...
def show(pista):
    pista_reservas = [reserva for reserva in reservas.values() if
                      reserva['pista'] == int(pista)]  # Filtramos las reservas de la pista especificada
